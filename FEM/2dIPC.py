...
class System:
    ...
    def implicit_U5(self, i):
        ...
        return -1 * ti.log(min(ti.abs(self.node[i]).x, 1e-2)) + -1 * ti.log(min(ti.abs(self.node[i].y), 1e-2))
    ...
    def implicit_prepare_node(self):
        ...
        for i in range(self.vn_object_index[self.count]):
            ...
            self.prev_node[i] *= 0.5
        # print("u0", self.U0(0) * self.dt * self.dt)
